...
const Header = () => {
  ...
  const scrollToSection = (sectionId) => {
    if (sectionId === '#') {
      return; // Do nothing for empty resources link
    }
    const element = document.getElementById(sectionId);
    if (element) {
      element.scrollIntoView({
  ...
  return (
    <>
      <header className="absolute top-0 left-0 right-0 z-50 px-4 pt-6">
        <style jsx global>{keyframesStyle}</style>
        <div className="w-full max-w-3xl mx-auto bg-white/90 backdrop-blur-sm rounded-lg px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between">
          <div className="flex items-center">
            <Link href="/" className="flex items-center">
              <span className="text-2xl font-bold">H<span className="text-orange-500 text-3xl">U</span>STLE<span className="text-orange-500 ml-1">90</span></span>
            </Link>
          </div>
